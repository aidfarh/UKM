...
case "$1" in
	...
	DefaultGPUGovernor)
		...
		if [ "$POLICY" = "trustzone" ]; then
			$BB echo "`$BB cat /sys/devices/platform/kgsl-3d0.0/kgsl/kgsl-3d0/pwrscale/policy_config/$POLICY/governor`"
		else
			$BB echo $POLICY;
		fi;
	;;
	...
	;;
	DirGPUGovernor)
		$BB echo "/sys/devices/platform/kgsl-3d0.0/kgsl/kgsl-3d0/pwrscale/policy_config/trustzone/governor";
	;;
	DirGPUMaxFrequency)
	...
	;;
	DirIOSchedulerExt)
		$BB echo "/sys/block/mmcblk1/queue/scheduler";
	;;
	DirIOSchedulerTree)
		$BB echo "/sys/block/mmcblk0/queue/iosched";
	;;
	DirIOSchedulerTreeExt)
		$BB echo "/sys/block/mmcblk1/queue/iosched";
	;;
	DirTCPCongestion)
